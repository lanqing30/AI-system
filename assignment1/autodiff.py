...
class OnesLikeOp(Op):
    ...
    def __call__(self, node):
        ...
        new_node.name = "OnesLike({})".format(node.name)
        return new_node
...
class ZerosLikeOp(Op):
    ...
    def __call__(self, node):
        new_node = Op.__call__(self)
        new_node.inputs = [node]
        new_node.name = "ZerosLike({})".format(node.name)
        return new_node
...
class MulByConstOp(Op):
    ...
    def __call__(self, node, const_val):
        new_node = Op.__call__(self)
        new_node.const_attr = const_val
        new_node.name = "({0}*{1})".format(node, const_val)
        new_node.inputs = [node]
        return new_node
...
class MatMulOp(Op):
    ...
    def __call__(self, node_A, node_B, trans_A=False, trans_B=False):
        ...
        """
        new_node = Op.__call__(self)
        new_node.name = "MatMul({0}, {1}, {2}, {3})".format(node_A.name, node_B.name, trans_A, trans_B)
